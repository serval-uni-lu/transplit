...
from models import Informer, Autoformer, Transformer, Reformer, Transplit
...
class Exp_Main(Exp_Basic):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, _, _) in enumerate(train_loader):
                ...
                if (i + 1) % self.args.save_every == 0:
                    print("Saving model...")
                    torch.save(self.model.state_dict(), path + '/' + f"save_epoch{epoch}_iter{i}.pth")
    ...
    def test(self, setting, checkpoint=""):
        ...
        if not os.path.exists(folder_path):
            ...
            folder_path = f"{folder_path}_{len(os.listdir('./test_results/'))}/"
            os.makedirs(folder_path)
        folder_path += '/'
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, mean, scale) in enumerate(test_loader):
                ...
                outputs = outputs[:, -self.args.pred_len:, f_dim:]
                batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                if self.args.unscale:
                    outputs = outputs.detach().cpu() * scale + mean
                    batch_y = batch_y.detach().cpu() * scale + mean
                ...
                if (i + 1) % 1000 == 0:
                    print(f"[{i}] loss:", np.mean((np.array(preds) - np.array(trues))**2))
    ...
    def preds_from_array(self, checkpoint, X, y):
        # todo
        pass
